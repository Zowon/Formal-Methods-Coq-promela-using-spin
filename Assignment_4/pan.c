...
short src_ln1 [] = {
	 ...
	 10,  40,  41,  11,  11,  11,  11,  11, 
	 11,  11,  11,  47,  49,  49,  48,  51, 
	 36,  52,  36,  52,   0, };
S_F_MAP src_file1 [] = {
	{ "-", 0, 0 },
	{ "p22-9349_pc.pml", 1, 27 },
	{ "-", 28, 29 }
};
uchar reached1 [] = {
	  0,   1,   0,   1,   1,   1,   0,   1, 
...
short src_ln0 [] = {
	 ...
	 20,  21,  11,  11,  11,  11,  11,  11, 
	 11,  11,  27,  29,  29,  28,  31,  15, 
	 32,  15,  32,   0, };
S_F_MAP src_file0 [] = {
	{ "-", 0, 0 },
	{ "p22-9349_pc.pml", 1, 34 },
	{ "-", 35, 36 }
};
uchar reached0 [] = {
	  0,   1,   1,   0,   1,   0,   1,   1, 
...
{	int j = 0, h = now._nr_pr;
	...
	case 1:	/* Consumer */
		...
		((P1 *)pptr(h))->item = 0;
#ifdef VAR_RANGES
		logval("Consumer:item", ((P1 *)pptr(h))->item);
#endif
#ifdef HAS_CODE
		locinit1(h);
#endif
	...
	case 0:	/* Producer */
		...
		((P0 *)pptr(h))->item = 0;
#ifdef VAR_RANGES
		logval("Producer:item", ((P0 *)pptr(h))->item);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
{
		now.buffer = addqueue(calling_pid, 1, 0);
		now.sync = addqueue(calling_pid, 2, 0);
		now.isFull = 0;
		now.isEmpty = 5;
		now.mutex = 1;
#ifdef VAR_RANGES
...
{	int i = 0;
	...
	{	if (t->ty[i] == TIMEOUT_F) return 1;
		...
		switch (t->qu[i]) {
		case 0: break;
		case 1: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.buffer) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.buffer) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.buffer) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.buffer) )))
			    ) return 0; break;
		case 2: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.sync) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.sync) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.sync) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.sync) )))
			    ) return 0; break;
...
{	/* int i; */
	printf("global vars:\n");
	printf("	int    isFull:	%d\n", now.isFull);
	printf("	int    isEmpty:	%d\n", now.isEmpty);
	printf("	int    mutex:	%d\n", now.mutex);
	printf("	chan buffer (=%d):	len %d:\t", now.buffer, q_len(now.buffer));
	c_chandump(now.buffer);
	printf("	chan sync (=%d):	len %d:\t", now.sync, q_len(now.sync));
	c_chandump(now.sync);
}
void
